<template>
    <div class="cart">
        ...
        <div class="cartFooter"  v-if="carts.length">
          <div class="Total"><span>合计：</span><span style="font-size: 0.54rem;color:#E3211E">￥{{sum}}</span></div>
          <div class="_box">
        ...
        <div class="cartMain">
             <ul>
                <li v-for="(cart,index) in carts" class="cartList">
                    ...
                    <div class="cartpurchase">
                             <a  @click="paya(cart)">立即购买</a>
                    </div>
...
export default {
  ...
  data(){
    ...
    methods:{
        ...
        paya:function(cart){
            console.log(cart )
            Toast({ message: `成功支付了${cart.value*cart.price}元`, iconClass: 'iconfont icon-goumaichenggong' ,duration: 750});
